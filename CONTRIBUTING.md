...
Engineering Kit v3.0 is the canonical baseline for future Codex work. It reflects completed work through Phase 3 Milestone 29: Private Beta and active Phase 3 Milestone 30 Beta Operations work. Milestone 30 is operations-only: Slice B documents deployment verification, smoke tests, rollback verification, monitoring verification, health verification, and log verification in `docs/04_IMPLEMENTATION/04-008_BETA_OPERATIONS_VERIFICATION.md`; Slice C documents the operator handbook, beta user handbook, invite documentation, onboarding workflow, and support guide in `docs/04_IMPLEMENTATION/04-009_BETA_OPERATOR_HANDBOOK.md`, `docs/04_IMPLEMENTATION/04-010_BETA_USER_HANDBOOK.md`, and `docs/04_IMPLEMENTATION/04-011_BETA_SUPPORT_GUIDE.md`; Slice D documents bug triage, feature requests, feedback review, production readiness, release, launch, and troubleshooting in `docs/04_IMPLEMENTATION/04-012_BETA_OPERATIONAL_WORKFLOWS.md` through `docs/04_IMPLEMENTATION/04-016_BETA_TROUBLESHOOTING_GUIDE.md`. Future tasks must consume the completed foundation, ranking contracts, API application boundary, dashboard application boundary, deterministic feedback API behavior, dashboard validation experience, design-partner walkthrough, and Private Beta policy instead of bypassing them.
...
During Phase 3 Milestone 30, these commands validate repository structure, documentation integrity, package boundaries, logging, event, database, domain, application, container, infrastructure composition, connector SDK foundation policy, connector runtime foundation policy, connector host foundation policy, Reddit connector foundation policy, Reddit runtime policy, Reddit provider transport boundary policy, Raw Content Pipeline Foundation policy, Normalization Pipeline Foundation policy, Embedding Foundation policy, LLM Analysis Foundation policy, Structured Analysis Foundation policy, Opportunity Engine Foundation policy, Opportunity Pipeline Foundation policy, Candidate Opportunity Engine policy, Opportunity Generation Workflow policy, Opportunity Ranking Engine policy, REST API foundation policy, Dashboard MVP policy, Product Validation Loop policy, Private Beta deployment readiness policy, Beta Operations policy, and package-level tests for `apps/api`, `apps/web`, `packages/config`, `packages/types`, `packages/errors`, `packages/utils`, `packages/shared`, `packages/events`, `packages/database`, `packages/domain`, `packages/application`, `packages/container`, `packages/infrastructure`, `packages/connectors`, `packages/connector-runtime`, `packages/connector-host`, `packages/connectors-reddit`, `packages/raw-content`, `packages/normalization`, `packages/embeddings`, `packages/llm-analysis`, `packages/analysis`, `packages/opportunity-engine`, `packages/opportunity-pipeline`, `packages/opportunity-candidates`, `packages/opportunity-generation`, and `packages/opportunity-ranking`.

Phase 3 Milestone 30 work must stay inside its scoped task. Milestone 30 may add Beta Operations documentation, repository verification, operational checklists, deployment verification guidance, smoke test guidance, rollback verification guidance, monitoring verification guidance, health verification guidance, log verification guidance, invite process verification guidance, onboarding verification guidance, support documentation, beta handbook, operator handbook, troubleshooting guidance, bug triage workflow, feature request workflow, and feedback review workflow. Slice B's deployment operations procedures are owned by `docs/04_IMPLEMENTATION/04-008_BETA_OPERATIONS_VERIFICATION.md`. Slice C's operator and beta-user procedures are owned by `docs/04_IMPLEMENTATION/04-009_BETA_OPERATOR_HANDBOOK.md`, `docs/04_IMPLEMENTATION/04-010_BETA_USER_HANDBOOK.md`, and `docs/04_IMPLEMENTATION/04-011_BETA_SUPPORT_GUIDE.md`. Slice D's workflows, checklists, and troubleshooting procedures are owned by `docs/04_IMPLEMENTATION/04-012_BETA_OPERATIONAL_WORKFLOWS.md`, `docs/04_IMPLEMENTATION/04-013_PRODUCTION_READINESS_CHECKLIST.md`, `docs/04_IMPLEMENTATION/04-014_RELEASE_CHECKLIST.md`, `docs/04_IMPLEMENTATION/04-015_LAUNCH_CHECKLIST.md`, and `docs/04_IMPLEMENTATION/04-016_BETA_TROUBLESHOOTING_GUIDE.md`. Slice E owns PR governance, documentation index, implementation order, and consistency verification. Every PR that changes Beta Operations material must complete the Beta Operations review in `.github/pull_request_template.md`. It is operations-only and must not add new backend features, AI features, new APIs, new dashboard features, new persistence features, new authentication features, payments, CRM integrations, notifications, analytics platforms, mobile apps, schedulers, workers, or unscoped product behavior.
...
Phase 3 Milestone 28 prepares Opportunity OS for design-partner validation. It is deterministic product validation only: feedback contracts, route behavior, dashboard interactions, and demo states must be explicit, testable, fixture-friendly, and safe to run without production services.

When Product Validation Loop files change, reviewers should confirm:

- feedback vocabulary stays deterministic and safe for design-partner walkthroughs
- API feedback exports route through `apps/api/src/index.ts`
- dashboard feedback work consumes the API boundary instead of bypassing it
- design-partner walkthroughs use `docs/04_IMPLEMENTATION/04-003_DESIGN_PARTNER_WALKTHROUGH.md`
- feedback values do not contain secrets, raw provider payloads, prompts, stacks, or unsafe internals
- no production persistence, billing, analytics platforms, notifications, email, CRM integration, scheduler, worker, mobile app, complex admin console, or unrelated product system was introduced
- `node scripts/verify-repository.mjs --phase review`, `node scripts/verify-repository.mjs --phase phase-3-milestone-28`, `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm build`, `pnpm test`, `pnpm --filter @opportunity-os/web test:e2e`, and `docker compose config` pass for the final readiness gate

## Phase 3 Milestone 28 Readiness

Before handing off to the next milestone, confirm:

- `apps/api` exposes deterministic feedback DTOs, validation, in-memory store behavior, route handlers, fixtures, integration tests, and security tests
- `apps/web` exposes deterministic save, dismiss, usefulness rating, evidence quality rating, ranking quality rating, reason category, feedback panel, validation summary, search/filter, fixture, unit, alignment, security, dependency-boundary, and Playwright coverage
- cross-app tests confirm dashboard feedback routes and vocabulary stay aligned with the API boundary
- design-partner walkthrough documentation is current and references only deterministic demo behavior
- no production persistence, billing, analytics platforms, notifications, email, CRM integration, scheduler, worker, mobile app, complex admin console, or unrelated product system exists
- the final readiness commands pass
...
Phase 3 Milestone 26 implements the REST API in `apps/api`. Future packages and apps must consume `apps/api` API contracts rather than redefining API envelopes, route definitions, request validation, pagination, filtering, error mapping, authentication contracts, authorization contracts, opportunity DTOs, ranking DTOs, fixtures, or API boundary tests.

When API files change, reviewers should confirm:

- public exports route through `apps/api/src/index.ts`
- dependencies remain limited to approved Opportunity OS upstream packages and deterministic test/build tooling
- route definitions, OpenAPI contracts, request/response envelopes, validation, pagination, filtering, and error mapping remain deterministic
- authentication and authorization remain contract-based unless a later milestone explicitly scopes a production provider
- opportunity and ranking routes use explicit ports and do not introduce persistence changes
- fixtures remain synthetic and contain no provider payloads, secrets, production examples, or real credentials
- errors and validation failures remain secret-safe and stack-safe by default
- integration, security, contract stability, dependency-boundary, lint, build, and test coverage pass
- no frontend implementation, billing, user management, analytics, notifications, production authentication provider, persistence change, scheduler, worker, provider SDK, or unrelated product workflow was introduced

## Phase 3 Milestone 26 Readiness

Before handing off to Phase 3 Milestone 27, confirm:
...
- API fixtures, integration tests, security tests, contract stability tests, and dependency-boundary tests cover the public API surface
- frontend implementation, billing, user management, analytics, notifications, production authentication providers, persistence changes, schedulers, workers, provider SDKs, and unrelated product workflows remain absent
- `node scripts/verify-repository.mjs --phase review`, `node scripts/verify-repository.mjs --phase phase-3-milestone-26`, `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm build`, `pnpm test`, and `docker compose config` pass
...
## Opportunity Pipeline Foundation Governance

Phase 2 Milestone 22 introduces the Opportunity Pipeline Foundation in `packages/opportunity-pipeline`. Future API, persistence, workflow, product, dashboard, scoring, ranking, and recommendation packages must consume `@opportunity-os/opportunity-pipeline` instead of redefining or bypassing pipeline contracts.

Opportunity Pipeline changes must confirm:
...
- pipeline primitives, stages, metadata, provenance, aggregation, assembly, candidate, validation, result, error, event, fixture, stability, security, dependency-boundary, and upstream integration contracts remain provider-independent
- fixtures use deterministic synthetic IDs, upstream opportunity references, validation examples, results, and events only
- security tests confirm no provider payloads, prompts, production business examples, API keys, credentials, tokens, auth headers, DSNs, database URLs, stack traces, raw causes, or dependency internals leak from safe outputs
- no business scoring algorithms, ranking algorithms, recommendation engines, REST APIs, frontend implementation, persistence implementation, schedulers, workers, provider SDKs, workflow engines, aggregation algorithms, generation logic, execution behavior, or business workflows are introduced

## Phase 2 Milestone 22 Readiness

Before handing off to the next milestone, confirm:

- `packages/opportunity-pipeline` is implemented, tested, documented, and independently buildable
- public exports route through `packages/opportunity-pipeline/src/index.ts`
...
- no business scoring algorithm, ranking algorithm, recommendation engine, REST API, frontend implementation, persistence implementation, scheduler, worker, provider SDK, workflow engine, aggregation algorithm, generation logic, execution behavior, or business workflow exists in `packages/opportunity-pipeline`
- `node scripts/verify-repository.mjs --phase review`, `node scripts/verify-repository.mjs --phase phase-2-milestone-22`, `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm build`, `pnpm test`, and `docker compose config` pass

Phase 2 Milestone 23 consumes `@opportunity-os/opportunity-pipeline` for provider-independent pipeline contracts.

## Candidate Opportunity Engine Governance

Phase 2 Milestone 23 introduces the Candidate Opportunity Engine in `packages/opportunity-candidates`. Future API, persistence, workflow, product, dashboard, scoring, ranking, and recommendation packages must consume `@opportunity-os/opportunity-candidates` instead of redefining or bypassing candidate opportunity contracts.

Candidate Opportunity Engine changes must confirm:

- public exports route through `packages/opportunity-candidates/src/index.ts`
- dependencies stay limited to approved upstream foundation packages
- candidate opportunity contracts, lifecycle contracts, metadata, provenance, evidence completeness, confidence aggregation, validation, results, safe errors, events, deterministic fixtures, stability tests, security tests, dependency-boundary tests, and upstream integration tests remain provider-independent
- fixtures remain synthetic and contain no provider payloads, prompts, secrets, or production business examples
- no production ranking algorithms, recommendation engines, business scoring, REST APIs, frontend implementation, persistence implementation, schedulers, workers, provider SDKs, or business workflows are introduced

## Phase 2 Milestone 23 Readiness

Before handing off to the next milestone, confirm:

- `packages/opportunity-candidates` is implemented, tested, documented, and independently buildable
- public exports route through `packages/opportunity-candidates/src/index.ts`
