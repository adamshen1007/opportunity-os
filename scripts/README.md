...
- External MVP Runtime checks during Phase 4 Milestone 34
- environment contract consistency between `.env.example`, `packages/config/src/schema.ts`, and the Engineering Kit variable set

Engineering Kit v3.0 treats the active `review` phase as the Phase 4 Milestone 34 boundary. Phase 3 Milestone 26 still has an explicit `phase-3-milestone-26` gate for REST API foundation work, Phase 3 Milestone 27 has an explicit `phase-3-milestone-27` gate for Dashboard MVP foundation work, Phase 3 Milestone 28 has an explicit `phase-3-milestone-28` gate for Product Validation Loop foundation work, Phase 3 Milestone 29 has an explicit `phase-3-milestone-29` gate for Private Beta deployment readiness, operations readiness, invite-only authentication, and persistence work, Phase 3 Milestone 30 has an explicit `phase-3-milestone-30` gate for Beta Operations policy, and Phase 4 Milestone 34 has an explicit `phase-4-milestone-34` gate for External MVP Runtime deployment readiness.
...
The Phase 4 Milestone 34 gate requires hosted external MVP runtime documentation, deployment workflow verification, hosted runtime environment variables, and production-safe API health output. It continues blocking YouTube/X/Product Hunt connectors, schedulers, workers, billing, CRM integrations, notifications, multi-tenancy, recommendation engines, complex admin consoles, and unsafe secret exposure.
...
The `review` phase now uses the active Phase 4 Milestone 34 boundary. Phase 1 Milestone 3 additionally verifies:
