...
Opportunity OS is currently an Engineering Kit, staged platform foundation, Product Behavior foundation, REST API foundation, Dashboard MVP, completed Product Validation Loop, completed Private Beta and Beta Operations foundations, completed Local Product Runtime, completed Product Data Schema, completed controlled Reddit Live Provider Transport, and active Phase 4 Milestone 34 External MVP Runtime. The repository contains shared foundation packages, connector foundation packages, Reddit provider transport contracts, deterministic Reddit runtime support, Raw Content Pipeline Foundation contracts, Normalization Pipeline Foundation contracts, Embedding Foundation contracts, LLM Analysis Foundation contracts, Structured Analysis Foundation contracts, Opportunity Engine Foundation contracts, Opportunity Pipeline Foundation contracts, Candidate Opportunity Engine Foundation contracts, Opportunity Generation Workflow Foundation contracts, the Opportunity Ranking Engine, the completed `apps/api` REST API application boundary, the completed `apps/web` Dashboard MVP application boundary, deterministic product validation feedback API behavior, dashboard feedback interactions, demo-ready validation states, design-partner walkthrough documentation, production readiness documentation, a local HTTP runtime for walking the dashboard against the API, and hosted external MVP deployment readiness policy. Milestone 34 prepares hosted web/API runtime, production environment binding, production-safe health checks, and external URL verification without adding YouTube/X/Product Hunt connectors, schedulers, workers, billing, CRM integrations, notifications, multi-tenancy, recommendation engines, or complex admin consoles.
...
Use `docs/04_IMPLEMENTATION/04-019_MVP_USER_TEST_READINESS.md` before asking a normal user to try Opportunity OS. The MVP trial should let a user understand the product purpose, review a ranked opportunity, inspect evidence and provenance, save or dismiss the opportunity, rate usefulness/evidence/ranking quality, and recover from loading, empty, and safe error states without developer help.

Use `docs/04_IMPLEMENTATION/04-020_MVP_TRIAL_WALKTHROUGH.md` for the exact local trial script, expected browser checkpoints, common failure cases, and pass/fail checklist.

The local MVP uses deterministic demo data by default. Controlled live Reddit provider access is optional, development-gated, and documented separately in `docs/04_IMPLEMENTATION/04-018_REDDIT_LIVE_PROVIDER_TRANSPORT.md`.
