...
- hosted external MVP runtime: `OPPORTUNITY_OS_API_URL`, `OPPORTUNITY_OS_WEB_URL`, `OPPORTUNITY_OS_WEB_ORIGINS`, `NEXT_PUBLIC_OPPORTUNITY_OS_API_BASE_URL`, `API_AUTH_REQUIRED`, `API_ADMIN_ACCESS_TOKEN`, `AUTH_SECRET_PEPPER`, `AUTH_INVITE_TTL_MS`, `AUTH_SESSION_TTL_MS`, `LLM_PROVIDER`, `LLM_MODEL`, `LLM_LIVE_ANALYSIS_ENABLED`, `LLM_PROVIDER_TIMEOUT_MS`
- multi-source validation: `STACK_EXCHANGE_LIVE_SCAN_ENABLED`, `STACK_EXCHANGE_API_BASE_URL`, `STACK_EXCHANGE_DEFAULT_SITE`, optional secret `STACK_EXCHANGE_API_KEY`, `STACK_EXCHANGE_TIMEOUT_MS`, and `STACK_EXCHANGE_QUERY`
...
- `LLM_PROVIDER_TIMEOUT_MS` must be an integer from `1` through `120000` when present.
