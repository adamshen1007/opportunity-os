...
const PROHIBITED_SOURCE_PATTERN =
  /\b(REST API|route handler|PrismaClient|persistGeneration|scheduleGeneration|WorkerProcess|live AI call|callLlm|callLLM|invokeModel|generateText|runPrompt|executePrompt|renderPrompt|billing|Stripe|UserAccount|authentication|authorization|scoreGeneration|rankGeneration|recommendGeneration|workflow engine|executeWorkflow)\b/iu;
...
describe("Opportunity Generation dependency boundaries", () => {
  it("depends only on approved upstream contract packages and test/build tooling", () => {
    ...
    for (const dependencyName of dependencyNames) {
      expect(
        ...
        `${dependencyName} is not an approved opportunity generation dependency`
      ).toBe(true);
